...
export function synchronizeExternalLatexImage(image: HTMLImageElement, dark: boolean) {
  const currentSource =
  ...
  const storedCanonical = normalizeExternalLatexUrl(image.getAttribute(SOURCE_DATA_ATTRIBUTE));
  const canonical =
    storedCanonical && storedCanonical === currentCanonical ? storedCanonical : currentCanonical;
