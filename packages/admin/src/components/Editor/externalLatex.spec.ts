...
describe('external LaTeX image theme URLs', () => {
  it('uses the existing renderer and adds its dark foreground parameter', () => {
    ...
    expect(getExternalLatexThemeUrl('//tex.xumin.net/svgb/encoded?cache=1', true)).toBe(
  ...
  it('restores the color-neutral URL in the light theme', () => {
    expect(
      getExternalLatexThemeUrl('https://tex.xumin.net/svg/x%5E2?cache=1&color=fff&c=000', false),
