...
export async function migrateWalineComments() {
  return request('/api/admin/comment/migration/waline', {
    method: 'POST',
  });
}
...
export async function getComments({ page = 1, pageSize = 10, status, keyword } = {}) {
  const query = [
    ['page', page],
    ['pageSize', pageSize],
    ['status', status],
  ...
  return request(`/api/admin/comment?${query}`, {
    method: 'GET',
  });
}

export async function updateComment(id, data) {
  return request(`/api/admin/comment/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    data,
  });
}

export async function replyComment(id, content) {
  return request(`/api/admin/comment/${encodeURIComponent(id)}/reply`, {
    method: 'POST',
    data: { content },
  });
}
...
export async function purgeComment(id) {
  return request(`/api/admin/comment/${encodeURIComponent(id)}/permanent`, {
    method: 'DELETE',
  });
}
