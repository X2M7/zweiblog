...
迁移 `/var/zweiblog` 目录到新机器，然后运行脚本重启服务即可。
...
`docker` 部署的 ZweiBlog 所有的状态都存储在持久化目录中，所以只需要备份/迁移持久化目录。
...
ZweiBlog 后台内置有 [导入导出](../advanced/backup.md) 功能，可以通过此功能实现备份。但当前有一些局限：
