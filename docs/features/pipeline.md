...
流水线能够在服务端执行代码，因此生产环境默认禁用执行。确认脚本与依赖来源可信后，部署者需要在 `docker-compose/.env` 中把 `ZWEI_BLOG_PIPELINE_ALLOW_UNSAFE_EXECUTION` 精确设为小写 `true`，并重新创建 ZweiBlog 容器。仅在后台启用流水线不能绕过这个生产环境开关。
