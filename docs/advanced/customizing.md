...
ZweiBlog 通过定制化功能允许你在页面中嵌入自己的 HTML、CSS、JS 片段。
...
生产环境默认不执行这里的脚本。部署者需要在 `docker-compose/.env` 中把运行时开关精确设为 `ZWEI_BLOG_ALLOW_TRUSTED_CUSTOM_CODE=true`，然后重新创建 ZweiBlog 容器。仅在后台开启“定制化”或保存脚本不会绕过该部署开关。

该脚本与博客前台同源，能够读取页面内容和浏览器存储，也能以当前访客身份发起请求。只应运行您完全信任并审计过的代码。这个开关只控制整站定制化脚本，不影响 `/c/...` 自定义页面自己的隔离/可信运行模式。
